...
function ulx.addmoney(calling_ply, target_plys, amt)
	...
	ulx.fancyLogAdmin(calling_ply, "#A gave " .. string.format(BaseWars.LANG.CURFORMER, "#s") .. " to #T", affected_plys, amt)
...
function ulx.takemoney(calling_ply, target_plys, amt)
	...
	ulx.fancyLogAdmin(calling_ply, "#A took " .. string.format(BaseWars.LANG.CURFORMER, "#s") .. " from #T", affected_plys, amt)
...
function ulx.setmoney(calling_ply, target_plys, amt)
	...
	ulx.fancyLogAdmin(calling_ply, "#A set #T's money to " .. string.format(BaseWars.LANG.CURFORMER, "#s"), affected_plys, amt)
...
local addlevel = ulx.command(CATEGORY_NAME, "ulx addlevel", ulx.addlevel, "!addlevel")
	...
	addlevel:addParam{type = ULib.cmds.NumArg, min = 0, max = BaseWars.Config.LevelSettings.MaxLevel, hint = "Level", ULib.cmds.round}
...
local setlevel = ulx.command(CATEGORY_NAME, "ulx setlevel", ulx.setlevel, "!setlevel")
	...
	setlevel:addParam{type = ULib.cmds.NumArg, min = 0, max = BaseWars.Config.LevelSettings.MaxLevel, hint = "Level", ULib.cmds.round}
