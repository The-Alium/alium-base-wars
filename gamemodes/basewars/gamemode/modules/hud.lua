...
function MODULE:Paint()
	...
	if hW > 0.01 then
		...
		draw.RoundedBox(0, 64 + 24, sH - 128 - 32 - 4, hW, pbarH, col1)
		draw.RoundedBox(0, 64 + 24 - nhW + pbarW, sH - 128 - 32 - 4, nhW, pbarH, col2)

	else

		draw.RoundedBox(0, 64 + 24, sH - 128 - 32 - 4, pbarW, pbarH, col2)
	...
	if me.Stuck and me:Stuck() and me:GetMoveType() == MOVETYPE_WALK then
		...
		if CurTime() > StuckTime + 1 then
			...
			draw.DrawText(stuckstr, tag .. ".Large", sW / 2 + 2, sH / 2 + 2, shade, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
