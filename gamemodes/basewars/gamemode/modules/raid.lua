...
function MODULE:CheckRaidable(ply, nocool, fac)
	...
	if not nocool then
		...
		if Cool < BaseWars.Config.Raid.CoolDownTime then
			return false, BaseWars.LANG.OnCoolDown .. " (" .. Left .. " " .. BaseWars.LANG.Seconds .. " " .. BaseWars.LANG.Remaining .. ")"
		end
...
function MODULE:GetVersus()
	...
	if not self:CheckForNULL() then
		...
		return BaseWars.LANG.NoFaction, BaseWars.LANG.NoFaction
...
function MODULE:CLIENT_SetupRaid(t, versus, versus2, faction)
	...
	PaintVS = name1 .. BaseWars.LANG.VS .. name2
	
	hook.Run("RaidStart", versus, versus2, faction)
...
function MODULE:Paint()
	...
	local text = tostring(TimeRemaining) .. BaseWars.LANG.SecondsShort .. " " .. BaseWars.LANG.Remaining
